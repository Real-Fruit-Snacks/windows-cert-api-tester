...
- **Import a Postman collection (`certapi import postman`)** — the format most teams already have: folders, both URL forms, query rows, headers (disabled stays disabled), raw/urlencoded/formdata bodies, and bearer/basic/apikey auth with request-level beating folder- and collection-level. `{{variables}}` share syntax and import unchanged; collection variables become an environment, with Postman's `secret` type staying secret (encrypted at rest). Anything that can't carry across is a named warning, never a silent drop.
...
- **Configuration profiles — stop retyping the tedious half** — put the certificate, proxy, revocation, retries, timeout, workspace, and standing headers in a `certapi.config.json` and run `certapi send https://api.internal/orders --profile corp` instead of a 200-character line. Found by walking up from the working directory (so a per-repository file works from anywhere inside it), or named with `--config`, or taken from `%APPDATA%`; `--no-config` ignores all of it for a reproducible run. **A typed flag always beats the profile**, and any value may be `${env:NAME}` so secrets live in the environment and the file stays safe to commit. `certapi config path|show|profiles` says exactly what is in effect — and prints `(set)` rather than a password.
- **`certapi doctor <url>` — why can't I reach this?** Makes the connection one stage at a time (URL → proxy decision → DNS → TCP → proxy tunnel → TLS handshake → HTTP) and reports **which stage broke**, timed, instead of a single error line. It surfaces things an ordinary request never can: **the certificate authorities the server accepts client certificates from, matched against the ones you actually have** ("none of yours are issued by any of those" is usually the whole answer), evidence that the network is **decrypting TLS in the middle** — which is why a client certificate can't get through — the proxy actually chosen for that URL including one a PAC script picked, and what the proxy said when it refused. When DNS or TCP fails it also checks whether the internet is reachable at all, distinguishing a **captive portal** from a host that just needs the VPN. `--json` for scripts.
- **`certapi proxy [<url>]` — which proxy do I actually get?** Prints the machine's proxy configuration (WPAD auto-detection, PAC script address, static proxy, bypass list) and, for a given URL, the answer from **Windows' own engine** — `WinHttpGetProxyForUrl`, running your PAC script exactly as the operating system does — beside the answer .NET gives, which is what certapi follows. When those two disagree you have the explanation for "it works in my browser but not here". A PAC script is JavaScript; nothing can predict its answer by reading settings, so this runs the real engine.
- **See what the network stack actually did** — `--trace` on any command reports .NET's own networking events as they happen (DNS, TCP connect, TLS handshake, connection established, request lifecycle), with `--trace-filter` to narrow the firehose and `--trace-file` to keep it. `--wire` goes a layer lower and prints **the plaintext bytes of the exchange** — the request exactly as it was framed and the response exactly as it arrived, hex and ASCII for anything that isn't text. That is the one thing a packet capture cannot give you on an encrypted connection without its keys, and it needs **no driver and no administrator rights**, because the tool is one end of the connection. On HTTP/2, `--frames` reads the same capture as frames — SETTINGS values, window updates, `RST_STREAM`, and the `GOAWAY` error code and debug data that explain a gateway hanging up. Credential headers are redacted in all three unless you ask otherwise.
- **"Am I actually reusing connections?"** — `certapi connections <url> -n 10` makes the requests and reports which connection each went out on: how many were opened, how many requests each served, and a plain verdict. Reusing a pooled connection skips a TCP and a TLS handshake, which against a remote endpoint is most of the time a small request takes — and it's otherwise invisible, because the responses look identical either way. `certapi bench --pool` reports the same for a load run, which is what makes its latency numbers readable.
- **Export to markdown notes (Obsidian, Logseq, a docs repo)** — `certapi export markdown -o <folder>` turns the workspace into a folder of linked notes, one per saved request plus environments and chains, with YAML frontmatter and `[[wikilinks]]`. **An Obsidian vault is just a folder of markdown files**, so there is no plugin to install and the same output works anywhere. `doctor --md` keeps a diagnosis — including the acceptable client-certificate authorities and any TLS-interception finding, verbatim — and `run --md` keeps a suite result with each failed assertion and what arrived instead, so a vault can chart a suite's health over time. Investigations and runs are append-only (a new note per run); the catalogue re-exports in place. Credentials, including ones hiding in a URL query string, are redacted by default — vaults sync.
...
- **Reach APIs through an SSH jump host** — `--proxy` speaks SOCKS as well as HTTP: run `ssh -D 1080 user@jump` and `--proxy socks5://127.0.0.1:1080` reaches whatever the jump host can reach, with **mutual TLS intact end to end** — SOCKS relays bytes and never terminates TLS, so the client certificate arrives at the real server, which no HTTP-inspecting middlebox can say. socks4/socks4a/socks5 accepted everywhere `--proxy` works, bypass list included.
- **Transport control per request** — override or bypass the proxy (or narrow it to specific hosts with a `--noproxy` bypass list), follow redirects or stop at the 3xx (with your own hop limit), turn automatic decompression off, and pin the HTTP version. It's a **Transport** tab in the app's request editor, saved with the request, and the same switches headless on `send`, `run`, and `fuzz`: `--no-redirect` / `--max-redirs <n>`, `--no-decompress`, `--http1.1` / `--http2` / `--http3` (HTTP/3 is QUIC — Windows 11/Server 2022+, direct connections only).
...
- **Live streaming (WebSocket & SSE)** — a *Stream* button opens a console that connects to a `ws://`/`wss://` endpoint (send messages, watch replies) or an `http(s)` `text/event-stream` endpoint (watch events arrive), reusing your selected client certificate. The `certapi ws` and `certapi sse` commands do the same headless — with the full transport story: pinned hosts need no `--insecure`, captured tokens attach automatically, and the same proxy/`--noproxy`/revocation flags as `send` apply. The OAuth fetch (`certapi token`) carries the same flags, plus the RFC 8628 **device-code flow** (`--grant device`) for signing in from a machine with no browser.
...
# why can't I reach this? staged triage, and what the server accepts client certs from
certapi doctor https://internal.corp/api/orders --cert "CN=matt"

# watch the stack, or read the actual bytes (no driver, no admin)
certapi send https://internal.corp/api/orders --cert "CN=matt" --trace
certapi send https://internal.corp/api/orders --cert "CN=matt" --wire
certapi send https://internal.corp/api/orders --cert "CN=matt" --http2 --frames

# am I actually reusing connections?
certapi connections https://internal.corp/api/orders --cert "CN=matt" -n 10

# keep the answer: notes for a vault, a docs repo, or a ticket
certapi export markdown -o C:\Users\me\Vault --index
certapi doctor https://internal.corp/health --md investigation.md
