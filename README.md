...
- **File uploads (multipart/form-data)** — `certapi send -F "field=value" -F "file=@path"` posts a multipart form and uploads files (curl-style; `-F` implies POST and `name=@path;type=<ct>` sets a part's content type), so you can test upload endpoints from the command line.
