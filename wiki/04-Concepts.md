...
Ordinary HTTPS authenticates the **server** to you: the server presents a certificate, your machine
checks it against trusted CAs. **Mutual** TLS adds the other direction — during the handshake, the
...
- **A file** — a `.pfx`/`.p12` (certificate + private key, optionally password-protected) or a PEM
  `.crt`/`.pem` (with the key inline or in a separate `--key-file`).
...
Presenting your certificate is separate from **trusting the server's** certificate. Internal APIs are
often behind a private CA your machine doesn't trust, which normally fails the handshake. The
**Ignore server certificate errors** toggle (`--insecure` on the CLI) bypasses that check. It's
...
saved session tokens, and your theme. By default it's a single JSON file at
...
- A **request** is a method + URL + params + headers + body + auth + certificate + timeout, plus any
...
The GUI and `certapi` are two faces of one core library. A request saved in the app runs headless
