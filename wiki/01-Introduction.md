...
**Certificate API Tester** is a desktop API client for Windows whose reason to exist is one thing the
mainstream tools handle poorly: **mutual TLS**. It authenticates HTTP requests with a client
certificate — either straight from the Windows certificate store or from a `.pfx`/`.pem` file — and
does it everywhere, in the GUI and the command line, without you writing a line of code.
...
- **`ApiTester.App.exe`** — a WPF desktop app: a full request builder, collections, environments,
  response views, assertions, discovery, streaming, and more.
- **`certapi.exe`** — a command-line client with the same capabilities, built for scripts, CI, and
  automation. Script-friendly exit codes; body to stdout, diagnostics to stderr.
...
- **Engineers testing internal / enterprise APIs** protected by client certificates, private CAs, or
...
- **AI agents** — via the built-in [MCP server](20-MCP-Server.md), an agent can make mTLS calls with a
...
- **mTLS everywhere** — every feature (send, run, fuzz, stream, OAuth, gateway, mock) can present a
  client certificate.
- **Windows-native auth** — honors your machine proxy (WPAD/PAC) with your Windows credentials, and
  supports [Windows Integrated Auth](08-Authentication.md) (Negotiate/NTLM).
...
authorization-code + PKCE) · **Windows Integrated Auth** · multipart uploads · GraphQL · endpoint
discovery (fuzzing) · **WebSocket & SSE** streaming · a local **mock server** · a local **mTLS
